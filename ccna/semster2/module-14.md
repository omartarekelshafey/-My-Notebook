...
الـ Router بيجمع المعلومات في جدول routingمن عدة مصادر:
...
* الRemote Networks: الشبكات البعيدة اللي الـ Router بيعرفها بطريقتين:
  * الStatic routes: بتضاف يدوياً &#x20;
  * الDynamic routing protocols: بتضاف لما الـ Protocols تتعلم عن الشبكات Dynamic.
...
1. الـ Router بيستلم الـ Data link frame وبيفك الـ IP packet.
...
4. بعدها بياخد واحد من 3 قرارات:
   * الForward to Directly Connected: لو الشبكة متصلة مباشرة، بيبعتها للجهاز فوراً.
   * الForward to Next-Hop Router: لو بعيدة، بيبعت الـ Packet للـ Router التالي المذكور في الـ Route entry.
...
* ال Interior Gateway Protocols (IGPs): دي البروتوكولات اللي بنستخدمها عشان نربط الـ Routers داخل مؤسسة أو شركة واحدة (زي OSPF و EIGRP و RIP).
* ال Exterior Gateway Protocols (EGPs): فيه بروتوكول واحد بس هو الـ BGP، وده وظيفته يربط الشركات الكبيرة ببعضها أو يربط الـ ISPs ببعضهم عبر الإنترنت.
...
| ---------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------- |
| RIP              | Hop Count                     | بيعد كام راوتر في السكة. لو طريق فيه راوترين وطريق فيه 3، هيختار الـ 2 حتى لو السكة أبطأ.                     |
| OSPF             | Cost                          | بيعتمد على الـ Bandwidth (السرعة) التراكمية. السكة اللي فيها لينكات أسرع بيكون الـ Cost بتاعها أقل، فبيفضلها. |
